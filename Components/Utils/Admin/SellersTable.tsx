...
import { Balance, Close, DeleteOutlined, Layers, OpenInNew } from "@mui/icons-material";
...
const SellersTable: React.FC<IAdminSeller> = ({ sellers, handleRefetch }) => {
  ...
  const handleOpenModal = (store: TAdminSeller, lastProducts: any, length: number, balance:number) => {
    setSelectedSeller({ store, lastProducts, length, balance })
    setOpenModal(true)
  }
        ...
        <Table sx={{ minWidth: 350 }} aria-label="stats table">
          ...
          <TableBody>
            {sellers
              ...
              ?.map(({ length, store, lastProducts, balance }, index) => (
                <TableRow
                  key={index}
                ...
                >
                  ...
                  <TableCell align="right">
                    <Layers
                      onClick={() => handleOpenModal(store, lastProducts, length, balance)}
                      className={"pointer"}
            ...
            <Grid container mb={2}>
              ...
              <Grid item xs={6}>
                <Stack spacing={1}>
                  ...
                  <Typography fontSize={14}>{getCurrency(selectedSeller?.store.currency) + " " + selectedSeller?.balance.toFixed(2)}</Typography>
                </Stack>
              </Grid>
            </Grid>
